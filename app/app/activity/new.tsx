...
export default function NewActivity() {
  ...
  const { create } = useActivities(username)
  const { groups, withId } = useGroups()
  const { baseDate } = useBaseDate()
  const [description, setDescription] = useState('')
  const [groupId, setGroupId] = useState<number>()
  const [started, setStarted] = useState(baseDate)
  ...
  const handleSubmit = async () => {
    if (description && groupId && started < ended) {
      await create({ description, started, ended, groupId })
      router.back()
    }
  }
